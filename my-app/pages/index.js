...
export default function Home() {
  ...
  const presaleMint = async () => {
    try {
      ...
      await tx.wait();
      setLoading(false);
      window.alert("You successfully minted an  Blockship NFT!");
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const publicMint = async () => {
    try {
      ...
      await tx.wait();
      setLoading(false);
      window.alert("You successfully minted a Blockship NFT!");
    } catch (err) {
      console.error(err);
    }
  };
...
// checks if the presale has started by quering the `presaleStarted`
 ...
 // checks if the presale has ended by quering the `presaleEnded`
  ...
  return (
    <div>
      <Head>
        <title>Blockship NFTs</title>
        <meta name="description" content="Whitelist-Dapp" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <div className={styles.main}>
        <div>
          <h1 className={styles.title}>Welcome to Blockship NFTs!</h1>
          <div className={styles.description}>
